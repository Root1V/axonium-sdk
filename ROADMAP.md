...
| AXO-83 | One host instead of two: the gateway issues tokens | ✅ | `auth_base_url` empty now means "wherever the gateway is". Pointing at a self-hosted deployment used to mean changing two addresses, and forgetting the second silently asked the official platform for a token to use elsewhere — nothing errored |
...
| AXO-85 | Bring the token endpoint into the shared corpus | 📋 | It is in no case: each runner mocks it per case rather than exercising it, so every token behaviour is covered three times by hand instead of once by contract. Found while writing AXO-84 |
| AXO-86 | Give Python and Rust the catalog-parity guard Go has | 📋 | Go refuses a catalog entry with no sentinel, and refuses one whose retryability disagrees. It caught both new token errors; Rust silently shipped one unretryable and Python would have too |
