...
| AXO-99 | Say what changed shape before publishing it, not after | ✅ | A tolerant accessor absorbs a type change without failing and without warning, so a consumer finds out from behaviour. `scripts/surface_diff.py` compares this tree against the wheel the index actually served and exits non-zero on a removal or re-type. Run against `rc3` it names the `tool_calls` change that reached Synaptum silently — four days before it did |
| AXO-100 | The wait test was right about the code and wrong about the machine | ✅ | It made the wall clock the oracle and allowed 40ms of overhead; a loaded machine spent 128ms on the mock server and three round trips, and it failed while `waited_for` was exactly correct. Now `initial_backoff == max_backoff`, so the total is an exact equality with no tolerance to tune |
| AXO-101 | Say what we do not know about a key after a failure | ✅ | Our text described what a key can *replay* and said nothing about what a failure does to it. A consumer with derived keys read the natural inference and built on it. Now stated as undefined, in all three SDKs and the docs, with what we measured and what we could not |
| AXO-102 | Ask the platform what a failed request does to its key | 📋 | Relayed to Prometheus with a measurement. Two questions: whether a failure holds the key for the window, and — if the error is stored deliberately — that it come back with its original type, since a stored `503` is retryable later and a stored `400` never is |
| AXO-103 | Re-vendor 2026-09-19b, and stop describing a gateway that no longer exists | ✅ | The platform closed both halves of what we reported: `X-RateLimit-Scope` now reaches the `429`, and that envelope stopped omitting `trace_id` — it is a strict superset of the standard one. Verified against a real `429` before believing it. Four of our own statements had become false. The body fallback stays, reframed: a deployment predating the fix still answers the old way, and the corpus now records both envelopes |
| AXO-104 | Our allowlist told callers `response_format` was unsupported for five days after it worked | ✅ | The platform announced it on 18/09 and we shipped `rc5` on the 20th still warning *«the gateway does not support response_format; it was not sent»* — false in both halves, and Python stripped the field so structured output was unreachable. Now a declared field in all three, verified end-to-end against the deployment |
| AXO-105 | Nothing tied the catalog to the guide it comes from | ✅ | The guard from `errors.json` to the three SDKs existed; the direction from the guide to `errors.json` did not, so a newly documented error could only go unnoticed. Its first run found `unknown-parameter`, which had been sitting in the re-vendored guide for days. Now mapped in all three, and the check refuses to pass by seeing nothing when the guide's format changes |
| AXO-106 | `request_kind` was documented as a set, and the set was wrong | ✅ | The platform warned that `predict` was coming and asked whether our types enumerate the kinds. They do not — `str` in all three, deliberately — but all three docstrings listed `"chat", "embeddings", "images"`: two pluralised wrongly, and `rerank` missing since we shipped it ourselves. Measured values now recorded, with a guard against the field ever becoming closed |
| AXO-107 | `POST /v1/models/{model}/predict`, and whether to type it | 📋 | A pass-through endpoint serving three modalities whose **body shape differs between engines within the same modality**. The platform recommends an uninterpreted `predict(model, body)` over a typed `classify(text)`, and offers to add `engine` to the catalog so `(modality, engine)` identifies a shape. They are waiting on our answer before doing it |
| AXO-108 | An error with no request id, on a response that carried one | ✅ | All three read `request_id` and `trace_id` from the body only. A `422` forwarded verbatim from a backend has neither in its body and both in its headers, so the caller got nothing to take to the platform team — on the errors where they most need it. Not specific to `/predict`: an HTML page from a proxy loses them the same way |
| AXO-97 | Every example in all three languages, and tabs that hold only code | ✅ | Fourteen groups showed one language; now none do. The tab grouping used `.*?</div>`, which backtracks, so one group swallowed a paragraph, a warning and an `<h2>` and rendered four tabs reading *Python, Python, Go, Rust*. Three tests hold both invariants, one of them against the grouping function itself — the page tests go green again once the corpus stops provoking the bug |
