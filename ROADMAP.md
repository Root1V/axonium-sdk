...
| AXO-82 | Expose `X-Idempotent-Replay-Of` in `meta` when the platform ships it | 📋 | A replay carries its own `request_id`, so today the id a caller holds cannot reach the row that explains the charge. The header closes that at call time without waiting for their usage migration, and they said it can ship first. All three already read `Idempotent-Replay`; this is the second half of the same field |
